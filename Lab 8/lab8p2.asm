...
LoopPS1:
	MOV AL, [SI]	; Load the next char to be printed - USING INPUT PARAMETER SI
	CMP AL, '$'		; Compare the char to '$'
	JE quitPS1		; If it is equal, then quit subroutine and return to calling code
	OUT DX,AL			; If it is not equal to '$', then print it
	DEC SI				; Point to the next char to be printed
	DEC SI
	jmp LoopPS1		; Jump back to the top of the loop
quitPS1:
	; Restore registers
	POP DX			; FIXED
	POP SI			; FIXED
...
gsPrint:
	...
	PUSH AX
											...
											; FIXED -- Load the starting address of the string

	CALL printRevStr		; FIXED -- Print the string in reverse
	CALL newLine				; FIXED
